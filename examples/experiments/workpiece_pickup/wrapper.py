...
class USBEnv(FrankaEnv):
    ...
    def init_cameras(self, name_serial_dict=None):
        ...
        physical_cameras = OrderedDict(
            (cam_name, kwargs)
            for cam_name, kwargs in name_serial_dict.items()
            if cam_name != "side_classifier"
        )
        resolved_cameras = RSCapture.resolve_camera_configs(physical_cameras)
        ...
        for cam_name, kwargs in name_serial_dict.items():
            if cam_name == "side_classifier":
                ...
                resolved_kwargs = resolved_cameras[cam_name]
                serial_number = resolved_kwargs["serial_number"]
                if serial_number in capture_by_serial:
                    cap = capture_by_serial[serial_number]
                else:
                    cap = VideoCapture(
                        RSCapture(name=cam_name, **resolved_kwargs)
                    )
